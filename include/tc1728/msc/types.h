/* (c) HighTec EDV-Systeme GmbH */
#ifndef _HAVE_TRICORE_MSC_TYPES_H_
#define _HAVE_TRICORE_MSC_TYPES_H_


/********************** MSC0_CLC_t *************************/
/*
   MSC0_CLC              "MSC0 Clock Control Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int DISR:1;              /*  = [0] = 0x00000001 */
      unsigned int DISS:1;              /*  = [1] = 0x00000002 */
      unsigned int SPEN:1;              /*  = [2] = 0x00000004 */
      unsigned int EDIS:1;              /*  = [3] = 0x00000008 */
      unsigned int SBWE:1;              /*  = [4] = 0x00000010 */
      unsigned int FSOE:1;              /*  = [5] = 0x00000020 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int _bit8:1;             /* 0 = [8] = 0x00000100 */
      unsigned int _bit9:1;             /* 0 = [9] = 0x00000200 */
      unsigned int _bit10:1;            /* 0 = [10] = 0x00000400 */
      unsigned int _bit11:1;            /* 0 = [11] = 0x00000800 */
      unsigned int _bit12:1;            /* 0 = [12] = 0x00001000 */
      unsigned int _bit13:1;            /* 0 = [13] = 0x00002000 */
      unsigned int _bit14:1;            /* 0 = [14] = 0x00004000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int _bit16:1;            /* 0 = [16] = 0x00010000 */
      unsigned int _bit17:1;            /* 0 = [17] = 0x00020000 */
      unsigned int _bit18:1;            /* 0 = [18] = 0x00040000 */
      unsigned int _bit19:1;            /* 0 = [19] = 0x00080000 */
      unsigned int _bit20:1;            /* 0 = [20] = 0x00100000 */
      unsigned int _bit21:1;            /* 0 = [21] = 0x00200000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_CLC_t_nonv;

typedef volatile MSC0_CLC_t_nonv MSC0_CLC_t;

/********************** MSC0_DC_t *************************/
/*
   MSC0_DC               "Downstream Command Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int DCL:16;              /*  = [0..15] = 0x0000ffff */
      unsigned int DCH:16;              /*  = [16..31] = 0xffff0000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DC_t_nonv;

typedef volatile MSC0_DC_t_nonv MSC0_DC_t;

/********************** MSC0_DD_t *************************/
/*
   MSC0_DD               "Downstream Data Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int DDL:16;              /*  = [0..15] = 0x0000ffff */
      unsigned int DDH:16;              /*  = [16..31] = 0xffff0000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DD_t_nonv;

typedef volatile MSC0_DD_t_nonv MSC0_DD_t;

/********************** MSC0_DSC_t *************************/
/*
   MSC0_DSC              "Downstream Control Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int TM:1;                /*  = [0] = 0x00000001 */
      unsigned int CP:1;                /*  = [1] = 0x00000002 */
      unsigned int DP:1;                /*  = [2] = 0x00000004 */
      unsigned int NDBL:5;              /*  = [3..7] = 0x000000f8 */
      unsigned int NDBH:5;              /*  = [8..12] = 0x00001f00 */
      unsigned int ENSELL:1;            /*  = [13] = 0x00002000 */
      unsigned int ENSELH:1;            /*  = [14] = 0x00004000 */
      unsigned int DSDIS:1;             /*  = [15] = 0x00008000 */
      unsigned int NBC:6;               /*  = [16..21] = 0x003f0000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int PPD:5;               /*  = [24..28] = 0x1f000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DSC_t_nonv;

typedef volatile MSC0_DSC_t_nonv MSC0_DSC_t;

/********************** MSC0_DSDSH_t *************************/
/*
   MSC0_DSDSH            "Downstream Select Data Source High Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int SH0:2;               /*  = [0..1] = 0x00000003 */
      unsigned int SH1:2;               /*  = [2..3] = 0x0000000c */
      unsigned int SH2:2;               /*  = [4..5] = 0x00000030 */
      unsigned int SH3:2;               /*  = [6..7] = 0x000000c0 */
      unsigned int SH4:2;               /*  = [8..9] = 0x00000300 */
      unsigned int SH5:2;               /*  = [10..11] = 0x00000c00 */
      unsigned int SH6:2;               /*  = [12..13] = 0x00003000 */
      unsigned int SH7:2;               /*  = [14..15] = 0x0000c000 */
      unsigned int SH8:2;               /*  = [16..17] = 0x00030000 */
      unsigned int SH9:2;               /*  = [18..19] = 0x000c0000 */
      unsigned int SH10:2;              /*  = [20..21] = 0x00300000 */
      unsigned int SH11:2;              /*  = [22..23] = 0x00c00000 */
      unsigned int SH12:2;              /*  = [24..25] = 0x03000000 */
      unsigned int SH13:2;              /*  = [26..27] = 0x0c000000 */
      unsigned int SH14:2;              /*  = [28..29] = 0x30000000 */
      unsigned int SH15:2;              /*  = [30..31] = 0xc0000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DSDSH_t_nonv;

typedef volatile MSC0_DSDSH_t_nonv MSC0_DSDSH_t;

/********************** MSC0_DSDSL_t *************************/
/*
   MSC0_DSDSL            "Downstream Select Data Source Low Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int SL0:2;               /*  = [0..1] = 0x00000003 */
      unsigned int SL1:2;               /*  = [2..3] = 0x0000000c */
      unsigned int SL2:2;               /*  = [4..5] = 0x00000030 */
      unsigned int SL3:2;               /*  = [6..7] = 0x000000c0 */
      unsigned int SL4:2;               /*  = [8..9] = 0x00000300 */
      unsigned int SL5:2;               /*  = [10..11] = 0x00000c00 */
      unsigned int SL6:2;               /*  = [12..13] = 0x00003000 */
      unsigned int SL7:2;               /*  = [14..15] = 0x0000c000 */
      unsigned int SL8:2;               /*  = [16..17] = 0x00030000 */
      unsigned int SL9:2;               /*  = [18..19] = 0x000c0000 */
      unsigned int SL10:2;              /*  = [20..21] = 0x00300000 */
      unsigned int SL11:2;              /*  = [22..23] = 0x00c00000 */
      unsigned int SL12:2;              /*  = [24..25] = 0x03000000 */
      unsigned int SL13:2;              /*  = [26..27] = 0x0c000000 */
      unsigned int SL14:2;              /*  = [28..29] = 0x30000000 */
      unsigned int SL15:2;              /*  = [30..31] = 0xc0000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DSDSL_t_nonv;

typedef volatile MSC0_DSDSL_t_nonv MSC0_DSDSL_t;

/********************** MSC0_DSS_t *************************/
/*
   MSC0_DSS              "Downstream Status Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int PFC:4;               /*  = [0..3] = 0x0000000f */
      unsigned int _bit4:1;             /* 0 = [4] = 0x00000010 */
      unsigned int _bit5:1;             /* 0 = [5] = 0x00000020 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int NPTF:4;              /*  = [8..11] = 0x00000f00 */
      unsigned int _bit12:1;            /* 0 = [12] = 0x00001000 */
      unsigned int _bit13:1;            /* 0 = [13] = 0x00002000 */
      unsigned int _bit14:1;            /* 0 = [14] = 0x00004000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int DC:7;                /*  = [16..22] = 0x007f0000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int DFA:1;               /*  = [24] = 0x01000000 */
      unsigned int CFA:1;               /*  = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_DSS_t_nonv;

typedef volatile MSC0_DSS_t_nonv MSC0_DSS_t;

/********************** MSC0_ESR_t *************************/
/*
   MSC0_ESR              "Emergency Stop Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int ENL0:1;              /*  = [0] = 0x00000001 */
      unsigned int ENL1:1;              /*  = [1] = 0x00000002 */
      unsigned int ENL2:1;              /*  = [2] = 0x00000004 */
      unsigned int ENL3:1;              /*  = [3] = 0x00000008 */
      unsigned int ENL4:1;              /*  = [4] = 0x00000010 */
      unsigned int ENL5:1;              /*  = [5] = 0x00000020 */
      unsigned int ENL6:1;              /*  = [6] = 0x00000040 */
      unsigned int ENL7:1;              /*  = [7] = 0x00000080 */
      unsigned int ENL8:1;              /*  = [8] = 0x00000100 */
      unsigned int ENL9:1;              /*  = [9] = 0x00000200 */
      unsigned int ENL10:1;             /*  = [10] = 0x00000400 */
      unsigned int ENL11:1;             /*  = [11] = 0x00000800 */
      unsigned int ENL12:1;             /*  = [12] = 0x00001000 */
      unsigned int ENL13:1;             /*  = [13] = 0x00002000 */
      unsigned int ENL14:1;             /*  = [14] = 0x00004000 */
      unsigned int ENL15:1;             /*  = [15] = 0x00008000 */
      unsigned int ENH0:1;              /*  = [16] = 0x00010000 */
      unsigned int ENH1:1;              /*  = [17] = 0x00020000 */
      unsigned int ENH2:1;              /*  = [18] = 0x00040000 */
      unsigned int ENH3:1;              /*  = [19] = 0x00080000 */
      unsigned int ENH4:1;              /*  = [20] = 0x00100000 */
      unsigned int ENH5:1;              /*  = [21] = 0x00200000 */
      unsigned int ENH6:1;              /*  = [22] = 0x00400000 */
      unsigned int ENH7:1;              /*  = [23] = 0x00800000 */
      unsigned int ENH8:1;              /*  = [24] = 0x01000000 */
      unsigned int ENH9:1;              /*  = [25] = 0x02000000 */
      unsigned int ENH10:1;             /*  = [26] = 0x04000000 */
      unsigned int ENH11:1;             /*  = [27] = 0x08000000 */
      unsigned int ENH12:1;             /*  = [28] = 0x10000000 */
      unsigned int ENH13:1;             /*  = [29] = 0x20000000 */
      unsigned int ENH14:1;             /*  = [30] = 0x40000000 */
      unsigned int ENH15:1;             /*  = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_ESR_t_nonv;

typedef volatile MSC0_ESR_t_nonv MSC0_ESR_t;

/********************** MSC0_FDR_t *************************/
/*
   MSC0_FDR              "MSC0 Fractional Divider Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int STEP:10;             /*  = [0..9] = 0x000003ff */
      unsigned int FDIS:1;              /*  = [10] = 0x00000400 */
      unsigned int SM:1;                /*  = [11] = 0x00000800 */
      unsigned int SC:2;                /*  = [12..13] = 0x00003000 */
      unsigned int DM:2;                /*  = [14..15] = 0x0000c000 */
      unsigned int RESULT:10;           /*  = [16..25] = 0x03ff0000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int SUSACK:1;            /*  = [28] = 0x10000000 */
      unsigned int SUSREQ:1;            /*  = [29] = 0x20000000 */
      unsigned int ENHW:1;              /*  = [30] = 0x40000000 */
      unsigned int DISCLK:1;            /*  = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_FDR_t_nonv;

typedef volatile MSC0_FDR_t_nonv MSC0_FDR_t;

/********************** MSC0_ICR_t *************************/
/*
   MSC0_ICR              "Interrupt Control Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int EDIP:2;              /*  = [0..1] = 0x00000003 */
      unsigned int EDIE:2;              /*  = [2..3] = 0x0000000c */
      unsigned int ECIP:2;              /*  = [4..5] = 0x00000030 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int ECIE:1;              /*  = [7] = 0x00000080 */
      unsigned int TFIP:2;              /*  = [8..9] = 0x00000300 */
      unsigned int _bit10:1;            /* 0 = [10] = 0x00000400 */
      unsigned int TFIE:1;              /*  = [11] = 0x00000800 */
      unsigned int RDIP:2;              /*  = [12..13] = 0x00003000 */
      unsigned int RDIE:2;              /*  = [14..15] = 0x0000c000 */
      unsigned int _bit16:1;            /* 0 = [16] = 0x00010000 */
      unsigned int _bit17:1;            /* 0 = [17] = 0x00020000 */
      unsigned int _bit18:1;            /* 0 = [18] = 0x00040000 */
      unsigned int _bit19:1;            /* 0 = [19] = 0x00080000 */
      unsigned int _bit20:1;            /* 0 = [20] = 0x00100000 */
      unsigned int _bit21:1;            /* 0 = [21] = 0x00200000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_ICR_t_nonv;

typedef volatile MSC0_ICR_t_nonv MSC0_ICR_t;

/********************** MSC0_ID_t *************************/
/*
   MSC0_ID               "Module Identification Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int MODREV:8;            /*  = [0..7] = 0x000000ff */
      unsigned int MODTYPE:8;           /*  = [8..15] = 0x0000ff00 */
      unsigned int MODNUM:16;           /*  = [16..31] = 0xffff0000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_ID_t_nonv;

typedef volatile MSC0_ID_t_nonv MSC0_ID_t;

/********************** MSC0_ISC_t *************************/
/*
   MSC0_ISC              "Interrupt Set Clear Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int CDEDI:1;             /*  = [0] = 0x00000001 */
      unsigned int CDECI:1;             /*  = [1] = 0x00000002 */
      unsigned int CDTFI:1;             /*  = [2] = 0x00000004 */
      unsigned int CURDI:1;             /*  = [3] = 0x00000008 */
      unsigned int CDP:1;               /*  = [4] = 0x00000010 */
      unsigned int CCP:1;               /*  = [5] = 0x00000020 */
      unsigned int CDDIS:1;             /*  = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int _bit8:1;             /* 0 = [8] = 0x00000100 */
      unsigned int _bit9:1;             /* 0 = [9] = 0x00000200 */
      unsigned int _bit10:1;            /* 0 = [10] = 0x00000400 */
      unsigned int _bit11:1;            /* 0 = [11] = 0x00000800 */
      unsigned int _bit12:1;            /* 0 = [12] = 0x00001000 */
      unsigned int _bit13:1;            /* 0 = [13] = 0x00002000 */
      unsigned int _bit14:1;            /* 0 = [14] = 0x00004000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int SDEDI:1;             /*  = [16] = 0x00010000 */
      unsigned int SDECI:1;             /*  = [17] = 0x00020000 */
      unsigned int SDTFI:1;             /*  = [18] = 0x00040000 */
      unsigned int SURDI:1;             /*  = [19] = 0x00080000 */
      unsigned int SDP:1;               /*  = [20] = 0x00100000 */
      unsigned int SCP:1;               /*  = [21] = 0x00200000 */
      unsigned int SDDIS:1;             /*  = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_ISC_t_nonv;

typedef volatile MSC0_ISC_t_nonv MSC0_ISC_t;

/********************** MSC0_ISR_t *************************/
/*
   MSC0_ISR              "Interrupt Status Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int DEDI:1;              /*  = [0] = 0x00000001 */
      unsigned int DECI:1;              /*  = [1] = 0x00000002 */
      unsigned int DTFI:1;              /*  = [2] = 0x00000004 */
      unsigned int URDI:1;              /*  = [3] = 0x00000008 */
      unsigned int _bit4:1;             /* 0 = [4] = 0x00000010 */
      unsigned int _bit5:1;             /* 0 = [5] = 0x00000020 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int _bit8:1;             /* 0 = [8] = 0x00000100 */
      unsigned int _bit9:1;             /* 0 = [9] = 0x00000200 */
      unsigned int _bit10:1;            /* 0 = [10] = 0x00000400 */
      unsigned int _bit11:1;            /* 0 = [11] = 0x00000800 */
      unsigned int _bit12:1;            /* 0 = [12] = 0x00001000 */
      unsigned int _bit13:1;            /* 0 = [13] = 0x00002000 */
      unsigned int _bit14:1;            /* 0 = [14] = 0x00004000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int _bit16:1;            /* 0 = [16] = 0x00010000 */
      unsigned int _bit17:1;            /* 0 = [17] = 0x00020000 */
      unsigned int _bit18:1;            /* 0 = [18] = 0x00040000 */
      unsigned int _bit19:1;            /* 0 = [19] = 0x00080000 */
      unsigned int _bit20:1;            /* 0 = [20] = 0x00100000 */
      unsigned int _bit21:1;            /* 0 = [21] = 0x00200000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_ISR_t_nonv;

typedef volatile MSC0_ISR_t_nonv MSC0_ISR_t;

/********************** MSC0_OCR_t *************************/
/*
   MSC0_OCR              "Output Control Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int CLP:1;               /*  = [0] = 0x00000001 */
      unsigned int SLP:1;               /*  = [1] = 0x00000002 */
      unsigned int CSLP:1;              /*  = [2] = 0x00000004 */
      unsigned int ILP:1;               /*  = [3] = 0x00000008 */
      unsigned int _bit4:1;             /* 0 = [4] = 0x00000010 */
      unsigned int _bit5:1;             /* 0 = [5] = 0x00000020 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int CLKCTRL:1;           /*  = [8] = 0x00000100 */
      unsigned int CSL:2;               /*  = [9..10] = 0x00000600 */
      unsigned int CSH:2;               /*  = [11..12] = 0x00001800 */
      unsigned int CSC:2;               /*  = [13..14] = 0x00006000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int SDISEL:3;            /*  = [16..18] = 0x00070000 */
      unsigned int _bit19:1;            /* 0 = [19] = 0x00080000 */
      unsigned int _bit20:1;            /* 0 = [20] = 0x00100000 */
      unsigned int _bit21:1;            /* 0 = [21] = 0x00200000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_OCR_t_nonv;

typedef volatile MSC0_OCR_t_nonv MSC0_OCR_t;

/********************** MSC0_USR_t *************************/
/*
   MSC0_USR              "Upstream Status Register"
*/
/**********************************************************/

typedef union { /* 32 bits */
   struct {
      unsigned int UFT:1;               /*  = [0] = 0x00000001 */
      unsigned int URR:3;               /*  = [1..3] = 0x0000000e */
      unsigned int PCTR:1;              /*  = [4] = 0x00000010 */
      unsigned int _bit5:1;             /* 0 = [5] = 0x00000020 */
      unsigned int _bit6:1;             /* 0 = [6] = 0x00000040 */
      unsigned int _bit7:1;             /* 0 = [7] = 0x00000080 */
      unsigned int _bit8:1;             /* 0 = [8] = 0x00000100 */
      unsigned int _bit9:1;             /* 0 = [9] = 0x00000200 */
      unsigned int _bit10:1;            /* 0 = [10] = 0x00000400 */
      unsigned int _bit11:1;            /* 0 = [11] = 0x00000800 */
      unsigned int _bit12:1;            /* 0 = [12] = 0x00001000 */
      unsigned int _bit13:1;            /* 0 = [13] = 0x00002000 */
      unsigned int _bit14:1;            /* 0 = [14] = 0x00004000 */
      unsigned int _bit15:1;            /* 0 = [15] = 0x00008000 */
      unsigned int UC:5;                /*  = [16..20] = 0x001f0000 */
      unsigned int _bit21:1;            /* 0 = [21] = 0x00200000 */
      unsigned int _bit22:1;            /* 0 = [22] = 0x00400000 */
      unsigned int _bit23:1;            /* 0 = [23] = 0x00800000 */
      unsigned int _bit24:1;            /* 0 = [24] = 0x01000000 */
      unsigned int _bit25:1;            /* 0 = [25] = 0x02000000 */
      unsigned int _bit26:1;            /* 0 = [26] = 0x04000000 */
      unsigned int _bit27:1;            /* 0 = [27] = 0x08000000 */
      unsigned int _bit28:1;            /* 0 = [28] = 0x10000000 */
      unsigned int _bit29:1;            /* 0 = [29] = 0x20000000 */
      unsigned int _bit30:1;            /* 0 = [30] = 0x40000000 */
      unsigned int _bit31:1;            /* 0 = [31] = 0x80000000 */
   } bits;
   unsigned long reg;
} __attribute__((aligned(4))) MSC0_USR_t_nonv;

typedef volatile MSC0_USR_t_nonv MSC0_USR_t;


#endif /* _HAVE_TRICORE_MSC_TYPES_H_ */

